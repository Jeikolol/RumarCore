var CustomerCheckHolderDatatableSelector = function (uniqueIdPrefix, formElemntsMapping) {
    ...
    this.fillInfoStep = function (mappedObject) {
        ...
        $(`${formElements.AccountNumber}`).val(formInfo.AccountNumber);
        $(`${formElements.BankCityName}`).val(formInfo.BankCityName);
        $(`${formElements.RoutingNumber}`).val(formInfo.RoutingNumber);
        $(`${formElements.CountryId}`).val(formInfo.BankCountryId).trigger("change");
        $(`${formElements.BankName}`).val(formInfo.BankName);
        ...
        $.get(`/CountryBank/GetCountryById?id=${formInfo.BankCountryId}`, function (data) {
            if (data) {
                var newOption = new Option(data.text, data.id, false, true);
                $(`${formElements.BankSelectorUniqueIdPrefix}-bank-country-id`).append(newOption).trigger('change');
            }
        });

        $.get(`/CountryBank/GetBankById?id=${formInfo.BankId}`, function (data) {
            if (data) {
                var newOption = new Option(data.text, data.id, false, true);
                $(`${formElements.BankSelectorUniqueIdPrefix}-bankId`).append(newOption).trigger('change');
            }
        });
        ...
        $("#clean-customer-check-holder-form-btn").on("click", function () {
            selft.cleanInfoStep();
        });
    ...
    this.cleanInfoStep = function () {
        ...
        $("#clean-customer-check-holder-form-btn").off("click");
    }
    ...
    function cleanFormElementsInfo() {
        ...
        $(`${formElements.AccountNumber}`).val("");
        $(`${formElements.BankCityName}`).val("");

        $(`${formElements.BankSelectorUniqueIdPrefix}-bankId`).val("").trigger("change");
        $(`${formElements.BankSelectorUniqueIdPrefix}-bank-country-id`).val("").trigger("change");
    ...
    function disableFormElementsInputs(booleanState) {
        ...
        $(`${formElements.BankSelectorUniqueIdPrefix}-bank-country-id`).attr("readonly", booleanState);
        $(`${formElements.BankSelectorUniqueIdPrefix}-bankId`).attr("readonly", booleanState);
        
        $(`${formElements.AccountNumber}`).attr("readonly", booleanState);
        $(`${formElements.BankCityName}`).attr("readonly", booleanState);

        $(`${formElements.RoutingNumber}`).attr("readonly", booleanState);
        $(`${formElements.CountryId}`).attr("readonly", booleanState);
        $(`${formElements.BankName}`).attr("readonly", booleanState);
        $("#search-by-account-number-btn").attr("disabled", booleanState);
    }
    ...
    function FormElements(mappingObject) {
        return {
            ...
            BankCityName: mappingObject.BankCityName,
            RoutingNumber: mappingObject.RoutingNumber,
            BankName: mappingObject.BankName,
            CountryId: mappingObject.CountryId
        }
    }
    // DO NOT TOUCH, ASK FIRST
    function FormInfo(mappingObject) {
        return {
            ...
            BankCountryId : mappingObject.BankCountryId,
            BankCityName : mappingObject.BankCityName,
            AccountNumber: mappingObject.AccountNumber,
